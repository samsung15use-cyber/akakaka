...
async def send_remaining_welcome_messages(user_id: int):
    """Отправляет приветственные сообщения 2, 3, 4 (без 1-го) с задержками"""
...
async def cmd_start(message: types.Message):
    ...
    # 1. ОТПРАВЛЯЕМ ПЕРВОЕ ПРИВЕТСТВИЕ (СРАЗУ)
    ...
    # 2. ЗАПУСКАЕМ ОСТАЛЬНЫЕ ПРИВЕТСТВИЯ (2,3,4) В ФОНЕ
    asyncio.create_task(send_remaining_welcome_messages(user_id))
    
    # 3. ПРОВЕРЯЕМ ПОДПИСКИ
    is_subscribed = await is_user_subscribed_to_all(user_id)
    
    # 4. ЕСЛИ ПОДПИСАН - ГЛАВНОЕ МЕНЮ
    if is_subscribed:
        if not user.get("bear_game_completed", False):
            user["bear_game_completed"] = True
            save_user(user_id, user)
    ...
    # 5. ЕСЛИ НЕ ПОДПИСАН - ИГРА
    if user.get("bear_game_completed", False):
        user["bear_game_completed"] = False
        save_user(user_id, user)
